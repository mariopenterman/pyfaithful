...
static bool is_infinite_while(const PycRef<ASTBlock>& blk)
{
...
static void strip_module_trailing_return(PycRef<ASTBlock> blk, bool inInfWhile = false)
{
...
static bool stmtEmitsScopeCode(const PycRef<ASTNode>& n)
{
    if (n == nullptr)
        return false;
    switch (n.type()) {
...
static bool leadingNopAlreadyHandled(const PycRef<ASTNode>& front)
{
    if (front == nullptr)
        return true;
    ...
    if (cleanBuild) {
        ...
        if (!(code->flags() & PycCode::CO_OPTIMIZED)) {
            for (const auto& n : clean->nodes()) {
                if (n != nullptr && n.type() == ASTNode::NODE_BLOCK)
                    strip_module_trailing_return(n.cast<ASTBlock>());
            }
        }
    }
